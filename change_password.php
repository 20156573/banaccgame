...
?>
    ...
    <div class="col-sm-10">
        <div class="you-right mx-5 pb-4">
            <h5 class="pt-4 pb-2 px-4">THAY ĐỔI MẬT KHẨU</h5>
            <hr>
            <?php foreach ($array as $key): ?>
            <form class="px-5" name="passwordForm" action="process_update_password.php" method="post">
                <input type="hidden" name="idCustomer"  value="<?php echo $key['idCustomer'] ?>">
                <div class="row py-3">
                    <input type="password" class="form-control col-sm-7" placeholder="Mật khẩu cũ" name="oldPassword">
                    <label class="col-sm-5"><?php if (isset($_GET['error'])) {echo "Mật khẩu của bạn không đúng.";}
                    ?></label>
                </div>
