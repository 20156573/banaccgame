...
WHERE bill.idBill is null and game.idGame='2' and ((game.name like '%$search%') or (acc.rank like '%$search%'))";

if(isset($_POST['option'])){
	$option = $_POST['option'];
	if ($option == '1') {
			$sql ="select *,acc.idAcc, bill.status from acc LEFT JOIN bill ON bill.idAcc=acc.idAcc
WHERE bill.idBill is null AND idGame='2' order by price DESC";
	}
	if ($option == '2') {
			$sql ="select *,acc.idAcc, bill.status from acc LEFT JOIN bill ON bill.idAcc=acc.idAcc
WHERE bill.idBill is null AND idGame='2' order by price ASC";
	} 
}
...
if(isset($_POST['submit'])){
	...
	$max = $_POST['max'].'000';
	$sql ="select *,acc.idAcc, bill.status from acc LEFT JOIN bill ON bill.idAcc=acc.idAcc
WHERE bill.idBill is null AND idGame='2' AND acc.price >= $min and acc.price <= $max order by price ASC";
...
<div class="row first-row ">
	...
	<div class="col-sm-2">
		<a class="nav-link text-center" href="index.php">
			<img class="img-fluid mx-auto d-block rounded-circle hoverable img-dif" src="logopubg.png">PUBG Moblie
		</a>
	</div>
	<div class="col-sm-2">
		<a class="nav-link text-center" href="lienquanmobile.php">
			<img class="img-fluid mx-auto d-block rounded-circle" src="logolienquanmobile.png"><h6>Liên Quân Mobile</h6>
		</a>	
	</div>
</div>
<hr class="mt-0">
